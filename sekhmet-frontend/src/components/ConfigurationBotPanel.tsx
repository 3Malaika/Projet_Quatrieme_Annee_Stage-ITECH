      ...
      <Card><CardContent className="p-6 space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <label className="space-y-1 text-sm">
            <span>Délai d'abandon d'une escalade (minutes)</span>
            <Input type="number" min={5} max={10080} value={cfg.escalations.autoCancelAfterMinutes ?? 180} onChange={e=>setCfg({...cfg, escalations:{...cfg.escalations,autoCancelAfterMinutes:Number(e.target.value)}})} />
            <span className="block text-xs text-muted-foreground">Passé ce délai sans réponse d'aucun collaborateur, l'escalade est annulée automatiquement plutôt que de rester en attente indéfiniment.</span>
          </label>
        </div>
